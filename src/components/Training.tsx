        ...
        >
          <div className="p-6 border-b border-gray-200 flex justify-between items-center">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-600">{selectedProgram.duration_hours}h • {selectedProgram.price}€</p>
              </div>
            </div>
            <button
          ...
          <div className="p-6 space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedProgram.duration_hours}h</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getLevelColor(selectedProgram.level)}`}>
                        {getLevelText(selectedProgram.level)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Participants max:</span>
                      <span className="font-medium">{selectedProgram.max_participants}</span>
                    </div>
                  </div>
                </div>

                <div className="bg-green-50 rounded-lg p-4">
                  <h5 className="font-medium text-green-900 mb-2">Financement</h5>
            ...
            {selectedProgram.objectives && selectedProgram.objectives.length > 0 && (
              <div>
                <h4 className="text-lg font-bold text-gray-900 mb-3">Objectifs pédagogiques</h4>
                <ul className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {selectedProgram.methods.map((method: string, index: number) => (
                    <div key={index} className="flex items-center p-3 bg-blue-50 rounded-lg">
                      ...
                      <span className="text-blue-800 text-sm">{method}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {selectedProgram.evaluation_methods && selectedProgram.evaluation_methods.length > 0 && (
              <div>
                <h4 className="text-lg font-bold text-gray-900 mb-3">Méthodes d'évaluation</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {selectedProgram.evaluation_methods.map((method: string, index: number) => (
                    <div key={index} className="flex items-center p-3 bg-purple-50 rounded-lg">
                      ...
                      <span className="text-purple-800 text-sm">{method}</span>
                    </div>
                  ))}
                </div>
              </div>
                      ...
                      {module.topics && module.topics.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {module.topics.map((topic: string, topicIndex: number) => (
                            <span key={topicIndex} className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-800">
                              {topic}
                            </span>
                          ))}
                        </div>
            ...
            {selectedProgram.certification_type && (
              <div>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="flex items-start">
                    <Award className="h-5 w-5 text-yellow-600 mr-3 mt-0.5" />
                    <div>
                      <p className="font-medium text-yellow-900">{selectedProgram.certification_type}</p>
                      {selectedProgram.certification_provider && (
                        <p className="text-yellow-800 text-sm">Délivré par: {selectedProgram.certification_provider}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            {selectedProgram.accessibility_info && (
              <div>
                <h4 className="text-lg font-bold text-gray-900 mb-3">Accessibilité</h4>
...
const getIconForCategory = (iconName: string) => {
    const iconMap = {
      ...
      <AnimatePresence>
        {showProgramModal && <ProgramDetailsModal />}
      </AnimatePresence>
    };
