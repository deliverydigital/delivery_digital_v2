 ...
 */
export function buildChecklist(company, entries, declarations = []) {
  ...
  const steps = [
    ...
    {
      key: 'tva',
      label: 'Rapprocher / vérifier la TVA',
      // Vérifié dès que TOUTES les lignes sont pointées en vert (clic), ou en franchise.
      // NB : le rapprochement se fait ici ; la DÉCLARATION se dépose sur impots.gouv.fr.
      done: company?.regime_tva === 'franchise' || allReviewed,
      detail: company?.regime_tva === 'franchise' ? 'Franchise en base (pas de TVA)'
        : (allReviewed ? 'TVA vérifiée — à déclarer sur impots.gouv.fr' : `Pointez vos lignes (clic vert) : ${reviewed}/${total} vérifiées`),
      blocking: false,
    },
    {
      key: 'cloture',
  ...
  // Étapes APRÈS la clôture : dépôts officiels à faire sur impots.gouv.fr (l'app ne
  // dépose pas à ta place). Déduites des déclarations de l'exercice ; statut "déposé"
  // suivi dans l'onglet Documents. @author Rabah Ziane - 2026-07-16
  const IMPOTS_PRO = 'https://www.impots.gouv.fr/professionnel';
  const cloture_faite = !!company?.cloture?.verrouille;
  const post_cloture = (declarations || []).map(d => ({
    key: 'decl_' + d.type,
    label: d.label,
    done: d.status === 'depose',
    detail: d.status === 'depose'
      ? `Déposé${d.depose_le ? ' le ' + new Date(d.depose_le).toLocaleDateString('fr-FR') : ''}`
      : (d.echeance ? `À déposer avant le ${new Date(d.echeance).toLocaleDateString('fr-FR')}` : 'À déposer'),
    link: IMPOTS_PRO,
  }));
