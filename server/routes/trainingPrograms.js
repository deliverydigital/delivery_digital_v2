...
// Get single training program (admin only)
router.get('/:programId', authenticate, authorize('admin'), async (req, res) => {
  try {
    ...
    console.log('📊 Sending single program response');
    res.json(responseData);

  } catch (error) {
    console.error('Get single training program error:', error);
    res.status(500).json({
      success: false,
