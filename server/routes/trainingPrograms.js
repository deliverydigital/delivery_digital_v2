  ...
  try {
    ...
    try {
      ...
      const programs = await TrainingProgram.find(query)
        .select('program_id title description category duration_hours price level max_participants is_active is_featured opco_eligible cpf_eligible certification_type modules access_delay')
        .skip(skip)
