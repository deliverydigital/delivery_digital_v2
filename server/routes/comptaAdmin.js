...
import { renderCompteResultatSynthetique, renderBilanSynthetique } from '../lib/comptaSyntheseSage.js';
...
// yearOverride : visualiser une autre année en LECTURE seule sans modifier
// l'exercice courant ni re-synchroniser Qonto (sélecteur d'année du dashboard).
// @author Rabah Ziane - 2026-07-18
function exerciceRange(company, yearOverride) {
  const y = yearOverride || company?.exercice?.annee_courante || new Date().getFullYear();
...
function exerciceQuery(company, yearOverride) {
  const { start, end } = exerciceRange(company, yearOverride);
  return { company_id: company._id, date: { $gte: start, $lte: end } };
}

// Année de lecture demandée par le front (?year=2024), 0 si non fournie/invalide.
...
router.get('/companies/:id/entries', async (req, res) => {
  const c = await ComptaCompany.findById(req.params.id);
  if (!c) return res.status(404).json({ error: 'introuvable' });
  const q = exerciceQuery(c, readYear(req)); // exercice courant, ou année demandée en lecture
...
router.get('/companies/:id/dashboard', async (req, res) => {
  const c = await ComptaCompany.findById(req.params.id);
  if (!c) return res.status(404).json({ error: 'introuvable' });
  const vyear = readYear(req);             // année visualisée (lecture seule), 0 = exercice courant
  const entries = await ComptaEntry.find(exerciceQuery(c, vyear));
  // Déclarations de l'exercice (pour la section "après clôture" de la checklist).
  await ensureDeclarations(c);
  const yr = vyear || c.exercice?.annee_courante;
  const dq = { company_id: c._id }; if (yr) dq.annee = yr;
  const declarations = await ComptaDeclaration.find(dq).sort({ ordre: 1 }).lean();
  res.json({
    company: c.toSafeJSON(),
    resultat: computeResultat(entries),
    tva: computeTva(entries, c.tva_credit_anterieur),
    checklist: buildChecklist(c, entries, declarations),
    counts: {
...
router.get('/companies/:id/liasse/pdf', async (req, res) => {
  ...
  try {
    ...
    const bilanN = computeBilan(c.toSafeJSON(), computeCases2033(entries), treso, entries);
...
// Bilan SYNTHÉTIQUE format cabinet (style Sage), équilibré via solde Qonto + report à nouveau. @Rabah 2026-07-18
router.get('/companies/:id/liasse/bilan-synthetique-pdf', async (req, res) => {
  const c = await ComptaCompany.findById(req.params.id).select('+qonto.secret_key');
  if (!c) return res.status(404).json({ error: 'introuvable' });
  const entries = await ComptaEntry.find(exerciceQuery(c));
  try {
    ...
    const bilan = computeBilan(c.toSafeJSON(), computeCases2033(entries), treso, entries);
    const pdf = await renderBilanSynthetique(c.toSafeJSON(), bilan);
    const slug = (c.name || 'entreprise').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="bilan-${slug}-${c.exercice?.annee_courante || ''}.pdf"`);
    res.send(pdf);
  } catch (e) {
    res.status(500).json({ error: 'bilan_pdf_echec', detail: e.message });
  }
});

// Compte de résultat SYNTHÉTIQUE format cabinet (deux colonnes, style Sage). @Rabah 2026-07-18
router.get('/companies/:id/liasse/cr-synthetique-pdf', async (req, res) => {
  const c = await ComptaCompany.findById(req.params.id);
  if (!c) return res.status(404).json({ error: 'introuvable' });
  const entries = await ComptaEntry.find(exerciceQuery(c));
  try {
    const pdf = await renderCompteResultatSynthetique(c.toSafeJSON(), entries);
    const slug = (c.name || 'entreprise').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="compte-resultat-${slug}-${c.exercice?.annee_courante || ''}.pdf"`);
    res.send(pdf);
  } catch (e) {
    res.status(500).json({ error: 'cr_pdf_echec', detail: e.message });
  }
});
